...
def fetch_markets_batch(offset, limit=500):
    """Fetch a batch of markets from the API with retry logic."""
...
def fetch_all_markets(max_workers=10):
    """Fetch ALL markets exhaustively using pagination."""
    print("Starting exhaustive market search...")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        while has_more:
            ...
            for current_offset, future in futures:
                try:
                    ...
                    if markets:
                        ...
                        print(f"Fetched {len(markets)} markets at offset {current_offset} (total: {len(all_markets)})")
                except Exception as e:
            ...
            if all_empty:
                has_more = False
                print("Reached the end of available markets.")
...
def get_top_markets_by_volume(top_n=500, max_workers=10):
    ...
    print("Filtering for election win markets...")
    filtered_markets = []
    ...
    print(f"Found {len(filtered_markets)} markets matching criteria.")
    ...
    print(f"Selected top {len(top_markets)} markets by volume out of {len(filtered_markets)} matches.")
...
def main():
    ...
    # Get ALL markets, filter them, and select top 500 by volume
